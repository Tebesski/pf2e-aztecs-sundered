        ...
        <div class="aztec-durability-summary" style="${containerStyle}">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 4px;">
                <span style="display: flex; align-items: center; gap: 12px;">
                    <span>${game.i18n.localize('PF2E.HitPointsShortLabel')}: <span class="durability-edit" data-item-id="${item.id}" data-flag-key="currentHp" contenteditable="true">${currentHp}</span> / <span class="durability-edit" data-item-id="${item.id}" data-flag-key="maxHp" contenteditable="true">${maxHp}</span></span>
            ...
            <div>
                <span>${game.i18n.localize('pf2e-aztecs-sundered.sheet-text.hardness')}: <span class="durability-edit" data-item-id="${item.id}" data-flag-key="hardness" contenteditable="true">${hardness}</span> (${game.i18n.localize('PF2E.Item.Physical.BrokenThreshold.ShortLabel')}: ${threshold})</span>
            </div>
        </div>
    `
}
      ...
      if (changes.system?.equipped) {
         ...
         if (!allowedTypes.includes(newCarry) || newInSlot === true) {
            ui.notifications.warn(
               game.i18n.format('pf2e-aztecs-sundered.notifications.cant-equip', {itemName: item.name})
            )
...
Hooks.on("renderActorSheet", (app, htmlElement, data) => {
   ...
   physicalItems.forEach((item) => {
      ...
      if (isDefaultType || hasDurability) {
         ...
         if (maxHp > 0) {
            if (currentHp <= 0) {
               let skullIcon = `<i class="fa-solid fa-skull" style="color: #555; margin-right: 6px;" title="${game.i18n.localize('PF2E.Item.Physical.Destroyed')}"></i>`
               nameElement.prepend(skullIcon)
               itemRow.css({ opacity: "0.5", filter: "grayscale(100%)" })
            } else if (currentHp <= threshold) {
               let brokenIcon = `<i class="fa-solid fa-heart-crack" style="color: #a83232; margin-right: 6px;" title="${game.i18n.localize('PF2E.condition.broken.name')}"></i>`
